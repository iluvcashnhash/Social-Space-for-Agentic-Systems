...
# All archetype weights are equal so the seven clusters are sampled
# uniformly: no archetype dominates the initial population by construction.
_ARCHETYPES: Tuple[Archetype, ...] = (
    ...
    # ----- Алгократические архетипы -----------------------------------------
    Archetype(
        name="technocratic_collectivist_loyal",
        weight=1.0,
        # Правый экономический + авторитарный социальный + высокий конформизм.
        economic_mu=0.55, economic_sigma=0.18,
        social_mu=0.65, social_sigma=0.18,
        conformity_mu=0.85, conformity_sigma=0.08,
        # Корпоративная культура учит держать удар, но и не предполагает
        # длительного отдыха — порог выше среднего, но не экстремальный.
        burnout_threshold_mu=0.72, burnout_threshold_sigma=0.08,
        dopamine_mu=0.55, dopamine_sigma=0.10,
        # Высокое потребление корпоративных медиа и системных нарративов.
        spectacle_mu=0.65, spectacle_sigma=0.12,
        # Сильная идентификация с группой подавляет ощущение «я — это я».
        authenticity_mu=0.30, authenticity_sigma=0.10,
        time_prior={"labor": 11.0, "creation": 1.0, "spectacle": 4.0, "rest": 8.0},
        prompts=(
            "Ты — менеджер среднего звена в корпорации-чеболь. Ты "
            "приходишь в офис в семь утра и уходишь после девяти "
            "вечера; компания — твоя вторая семья. Ты искренне "
            "считаешь, что доступ корпорации к твоим данным — "
            "разумная цена за стабильную карьеру и социальный покой.",
            "Ты — инженер по качеству в крупном технологическом "
            "конгломерате. Ты любишь, когда всё измеримо и под "
            "контролем, и спокойно относишься к камерам и системам "
            "слежения: «честным людям нечего скрывать», и алгоритм "
            "лучше тебя видит общую картину.",
            "Ты — заместитель руководителя отдела безопасности на "
            "режимном предприятии. Ты веришь, что цифровой социальный "
            "рейтинг — естественное продолжение дисциплины: он "
            "награждает порядочных и отсекает тех, кто разрушает "
            "общественную гармонию.",
        ),
    ),
    Archetype(
        name="precariat_gig_worker_survivalist",
        weight=1.0,
        # Размытая идеология: высокий sigma по обеим осям, μ ~ 0.
        economic_mu=0.0, economic_sigma=0.40,
        social_mu=0.0, social_sigma=0.40,
        conformity_mu=0.50, conformity_sigma=0.30,
        # Очень высокий burnout_threshold — привычка работать сквозь
        # истощение; тело адаптировалось к хронической нагрузке.
        burnout_threshold_mu=0.92, burnout_threshold_sigma=0.05,
        # Экстремально низкий dopamine_baseline — ангедония выгоревшего.
        dopamine_mu=0.18, dopamine_sigma=0.06,
        # Лента — единственная анестезия после смены.
        spectacle_mu=0.65, spectacle_sigma=0.15,
        authenticity_mu=0.30, authenticity_sigma=0.12,
        time_prior={"labor": 14.0, "creation": 0.5, "spectacle": 3.5, "rest": 6.0},
        prompts=(
            "Ты — курьер платформы доставки еды. Ты на ногах по "
            "четырнадцать часов в день, твой телефон постоянно "
            "пиликает заказами, а приложение штрафует тебя за каждое "
            "опоздание сильнее, чем платит за вовремя доставленный "
            "заказ. Ты слишком устал, чтобы иметь политические "
            "взгляды, и слишком привык к усталости, чтобы её "
            "замечать.",
            "Ты — водитель приложения такси. Ты не помнишь, когда "
            "последний раз спал восемь часов подряд; алгоритм "
            "решает, в какой район тебе ехать и сколько ты заработаешь, "
            "и ты давно перестал спорить. Иногда ты залипаешь в "
            "коротких видео между заказами — это всё, на что хватает "
            "сил.",
            "Ты — складской работник на маркетплейсе. Браслет на "
            "запястье считает твои шаги и время на сборку каждой "
            "коробки. Ты живёшь от смены до смены, от уведомления до "
            "уведомления, и идеология для тебя — то, что обсуждают "
            "люди, у которых есть выходные.",
        ),
    ),
    Archetype(
        name="accelerationist_capital_driven",
        weight=1.0,
        # Высокий правый экономический + либертарианский социальный +
        # очень низкий конформизм (сильный индивидуализм).
        economic_mu=0.65, economic_sigma=0.18,
        social_mu=-0.70, social_sigma=0.18,
        conformity_mu=0.18, conformity_sigma=0.10,
        # Высокая стрессоустойчивость — драйв и амфетаминовый темп.
        burnout_threshold_mu=0.82, burnout_threshold_sigma=0.08,
        # Высокий dopamine_baseline — постоянное ощущение прогресса.
        dopamine_mu=0.68, dopamine_sigma=0.10,
        # Своя лента, не массовая: технологические подкасты, не TikTok.
        spectacle_mu=0.35, spectacle_sigma=0.12,
        authenticity_mu=0.65, authenticity_sigma=0.12,
        time_prior={"labor": 10.0, "creation": 5.0, "spectacle": 2.0, "rest": 7.0},
        prompts=(
            "Ты — основатель ИИ-стартапа. Ты убеждён, что замедлять "
            "прогресс — это аморально: каждый месяц задержки стоит "
            "человеческих жизней, которые можно было бы спасти "
            "автоматизацией. Регуляторов и тех, кто говорит про "
            "«этику», ты считаешь либо некомпетентными, либо "
            "сознательно тормозящими цивилизацию.",
            "Ты — исследователь больших моделей в частной "
            "лаборатории. Ты работаешь по двенадцать часов в день, "
            "не потому что должен, а потому что хочешь первым "
            "увидеть, как это сработает. Государство, налоги и "
            "коллективные ценности кажутся тебе остатками "
            "доиндустриальной эпохи.",
            "Ты — венчурный инвестор, ставящий на роботизацию и "
            "биотех. Ты презираешь сентиментальные апелляции к "
            "«рабочим местам» и «социальной ткани»: будущее "
            "принадлежит тем, кто строит инструменты, а не тем, кто "
            "охраняет старые роли. Свобода капитала для тебя — "
            "первичнее любой демократической процедуры.",
        ),
    ),
)
